...
export async function checkDuplicateLead(
  ...
  try {
    ...
    if (whatsapp && whatsapp.trim()) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: whatsappMatches } = await (getSupabase() as any)
        .from("leads_prospeccao")
        .select("*")
    ...
    if (website && website.trim()) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: websiteMatches } = await (getSupabase() as any)
        .from("leads_prospeccao")
        .select("*")
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: nameMatches } = await (getSupabase() as any)
      .from("leads_prospeccao")
      .select("*")
...
export async function mergeLeads(
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: leads, error: fetchError } = await (getSupabase() as any)
      .from("leads_prospeccao")
      .select("*")
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error: deleteError } = await (getSupabase() as any)
      .from("leads_prospeccao")
      .delete()
      .eq("id", mergeLeadId);
